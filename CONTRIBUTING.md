...
| `PROBE_MEMO=1` | answer a probe from its memo while none of the lines it solved (the lines of the cells it settled) has had a committed change since: same outcome and fill, tree bit-for-bit the same (corpus gate 0 mismatches, probes -19.9%). Instructions 3867 (150k nodes) -5.3%, 7382 -3.8%, 12130 -0.4%, but cycles on a quiet core (medians of 7) 3867 +0.3%, 7382 +3.6%, 12130 +3.0%: it answers the short cache-hot probes and pays memo-line and stamp loads plus a 32-byte store per probe. Not shipped. Off under `--learn`: a probe's clause pass reads cells on lines outside the memo's touched set, so a memo can replay a contradiction whose clause antecedent has since been reverted and force a wrong cell |
